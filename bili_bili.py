#!/usr/bin/env python3
"""
bili_termux_downloader.py
Termux-friendly BiliBili downloader using yt-dlp as a Python library.

Features:
 - Prompts for a BiliBili URL.
 - Extracts available qualities (resolutions).
 - Lets user choose a quality (0 = best/highest).
 - Downloads the video (merges audio/video if needed using ffmpeg).
 - Saves to ~/storage/shared/Download by default (falls back to ~/downloads).
 - Optional: supply cookies file path for geo/login-restricted content.
"""
...
def default_output_dir():
    """
    Prefer the Termux shared downloads folder if it exists (termux-setup-storage),
...
def main():
    print("BiliBili downloader for Termux (uses yt-dlp).")
    ...
    if shutil.which('ffmpeg') is None:
        ...
        print("Install ffmpeg in Termux with: pkg install ffmpeg")
